    ...
    createViewNamingPart (record){
        ...
        return [
                ...
                listeners: {
                    change: function(elem, newValue, oldValue){
                        ...
                        if(newValue === 'document'){
                            ...
                            Ext.getCmp('published_only_objects').hide().setValue(false);
                            ...
                            data.config.classes = "";
                            Ext.getCmp('published_only_objects').show();
            ...
            new Ext.form.Checkbox({
                fieldLabel: t('plugin_opendxp_perspectiveeditor_published_only'),
                checked: data.config.where === 'objects.published = 1',
                id: 'published_only_objects',
                readOnly: this.readOnly,
                hidden: data.config.treetype !== 'object',
                listeners: {
                    change: function(elem, newValue, oldValue){
                        if (newValue) {
                            data.config.where = 'objects.published = 1';
                        } else if (data.config.where === 'objects.published = 1') {
                            delete data.config.where;
                        }
                        this.setDirty(true);
                    }.bind(this)
                }
